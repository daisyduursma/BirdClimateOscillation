...
alldat90$latRad<-abs(deg2rad(alldat90$lat))
alldat90$lonRad<-abs(deg2rad(alldat90$lon))
...
for(j in 1:length(fin_sp)){
  ...
  #tests if two samples of circular data come from same population
      ...
      lattStat<-round(watson.two.test(subset(spdat,SOI=="elNino",select= latRad),
                                      subset(spdat,SOI=="LaNina",select= latRad),alpha=0.05)$statistic,3)
      latpVal<-if ( lattStat > 0.385) {
        "< 0.001"} else 
          if ( lattStat > 0.268) {
            "< 0.01"} else 
              if ( lattStat >= 0.187) {
                "< 0.05"} else 
                {"> 0.05"}
      lonStat<-round(watson.two.test(subset(spdat,SOI=="elNino",select= lonRad),
                                      subset(spdat,SOI=="LaNina",select= lonRad),alpha=0.05)$statistic,3)
      lonpVal<-if ( lonStat > 0.385) {
        "< 0.001"} else 
          if ( lonStat > 0.268) {
            "< 0.01"} else 
              if ( lonStat >= 0.187) {
...
modDat[[j]]<-cbind(elN,laN,tStat,pVal,ObsCount)
}
...
for(j in 1:length(fin_sp)){
  spdat<-subset(alldat90,Scientific.Name==fin_sp[j])
  fit<-lm(formula = modifiedDOY ~ NOAA,data = spdat)
