...
#####No signfiicnat differnces between phases
start<-lmer(Quantile5~Phase+(1|Order),data=allDat)
Anova(start,test.statistic="F")
peak<-lmer(Quantile50~Phase+(1|Order),data=allDat)
Anova(peak,test.statistic="F")
conc<-lmer(Quantile95~Phase+(1|Order),data=allDat)
Anova(conc,test.statistic="F")
...
##########################################
ELstart<-data.frame(start=with(wide,Quantile5LaNina-Quantile5.elNino),
                     SOI="NINO")
