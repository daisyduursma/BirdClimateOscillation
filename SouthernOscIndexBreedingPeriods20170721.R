...
#BOM
SOI<-read.csv("/Users/daisy/Google Drive/PhD/ENSO/Data/BOM_SOI2.csv")
...
SOI<-SOI[,c("date","BreedingENSO3","Year","monthNumeric","month2")]
...
ModifiedDate$Phase <- factor(as.character(ModifiedDate$BreedingENSO3), levels = c("NINO", "NINA", "neutral"))
...
# # NULL model without fixed effects (but includes random effects)
# m0<-lmer(modifiedDOY ~1 + (1|Scientific.Name) + (1|Order),REML=T, data = ModifiedDate )
# summary(m0)
# r.squaredGLMM(m0)
#FULL - all fixed effects and random model
mF<-lmer(modifiedDOY ~ BreedingENSO3 + logelev*logdist + lat+(1|Acc)+(1|Scientific.Name)+(1|Order),REML=T, data = ModifiedDate )
summary(mF)
...
mFML<-lmer(modifiedDOY ~ BreedingENSO3 + lat+ logelev+logdist+ (1|Scientific.Name)+(1|Order),REML=F, data = ModifiedDate )
summary(mFML)
Anova(mFML,test.statistic="Chisq")


# 
# #R2c is "variance explained by fixed and random factors or variance explained by the entire model"
# library(visreg)
# visreg(mF, "lat", by="BreedingENSO", overlay=TRUE)
# visreg(mF, "logelev", by="BreedingENSO", overlay=TRUE)
# visreg(mF, "logdist", by="BreedingENSO", overlay=TRUE)
# visreg(mF, "logelev", by="BreedingENSO", type = "contrast")
...
par(mfrow = c(2,1))
hist(wide$ChangeBPEL)
hist(wide$ChangeBPELNeu)
...
lmChange<-lm(ChangeBPELNeu~modified95+(1|Accu.LaNina) ,data=wide)#EL to Nu
summary(lmChange)#end data explains about 16% if variation we observe, with those concluding earlier in the year
Anova(lmChange)
visreg(lmChange)
...
fit.season<-lm(ChangeBPEL~season ,data=wide)
summary(fit.season)
visreg(fit.season,"season", type = "conditional")
visreg(fit.season,"BreedingPeriod.Neutral", by="season")


#seasonal trend in %change in egg-laying comparing el nino to la nina
fit.seasonELtoLA<-lmer(ChangeBPEL~season+(1|Order) ,data=wide)
Anova(fit.seasonELtoLA, test.statistic="F")
#find differencs between regions
TukeyELtoLA<- glht(fit.seasonELtoLA, linfct=mcp(season="Tukey"))
summary(TukeyELtoLA)
...
#find differencs between regions
TukeyRegion2<- glht(fit.seasonLtoNU, linfct=mcp(season="Tukey"))
...
fit.season.length<-lmer(BreedingPeriod.elNino~season+(1|Order) ,data=wide)
...
#seasonal trend in length of egg-laying period
fit.season.lengthLA<-lmer(BreedingPeriod.LaNina~season+(1|Order) ,data=wide)
...
########################
#Simplified diagram of findings
...
visreg(mEle, "logelev",  trans = exp)



visreg(mFML, "lat", by="BreedingENSO", overlay=TRUE, partial=FALSE)




summary(subset(ModifiedDate), )
