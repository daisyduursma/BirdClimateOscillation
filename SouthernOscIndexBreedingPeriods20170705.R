...
#lmBP<-lm(formula = BreedingPeriod ~ Phase, data = allDat)
summary(lmBP)
...
write.csv(ModifiedDate,"/Users/daisy/Google Drive/PhD/Oscilations/Data/DaisyObs.csv",row.names = FALSE)
for(j in 1:length(fin_sp)){
  spdat<-subset(ModifiedDate,Scientific.Name==fin_sp[j])
  fit<-lm(formula = modifiedDOY~+elevation+distToCoast+lat,data = spdat)
  summary(fit)
