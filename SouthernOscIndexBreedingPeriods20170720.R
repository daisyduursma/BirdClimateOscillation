...
#BOM
SOI<-read.csv("/Users/daisy/Google Drive/PhD/ENSO/Data/BOM_SOI.csv")
...
SOI<-SOI[,c("date","BreedingENSO2","Year","monthNumeric","month2")]
...
ModifiedDate$Phase <- factor(as.character(ModifiedDate$BreedingENSO), levels = c("NINA", "NINO"))
...
mF<-lmer(modifiedDOY ~ BreedingENSO2 + logelev*logdist + lat+(1|Acc)+(1|Scientific.Name)+(1|Order),REML=T, data = ModifiedDate )
summary(mF)
...
lmChange<-lm(ChangeBPEL~modified95*BreedingPeriod.elNino ,data=wide)
...
fit.season<-lm(ChangeBPEL~BreedingPeriod.elNino*season ,data=wide)
summary(fit.season)
Anova((fit.season))
visreg(fit.season,"season", type = "conditional")
visreg(fit.season,"BreedingPeriod.elNino", by="season")
